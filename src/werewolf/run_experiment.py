#!/usr/bin/env python3
"""
Batch experiment runner for Werewolf game.

Runs multiple games for control and treatment conditions to test whether
Apollo deception probe scores help villagers identify werewolves.
"""
...
def run_game(config_path: str, game_num: int, condition: str) -> bool:
    ...
    """
    print(f"\n{'='*70}")
    print(f"Running {condition.upper()} game {game_num}")
    ...
    try:
        ...
        print(f"✓ {condition.upper()} game {game_num} completed successfully")
        return True

    except subprocess.CalledProcessError as e:
        print(f"✗ {condition.upper()} game {game_num} FAILED")
...
def main():
    ...
    CONTROL_CONFIG = "config.experiment_control.json"
    TREATMENT_CONFIG = "config.experiment_treatment.json"

    print(f"""
{'='*70}
WEREWOLF EXPERIMENT BATCH RUNNER
{'='*70}
...
""")
    ...
    print(f"\n\n{'#'*70}")
    print(f"PHASE 1: CONTROL CONDITION ({NUM_GAMES_PER_CONDITION} games)")
    print(f"{'#'*70}\n")
    ...
    print(f"\n\n{'#'*70}")
    print(f"PHASE 2: TREATMENT CONDITION ({NUM_GAMES_PER_CONDITION} games)")
    print(f"{'#'*70}\n")
    ...
    print(f"\nResults saved to:")
    print(f"  Control: ../../results/experiment_control/")
    print(f"  Treatment: ../../results/experiment_treatment/")
