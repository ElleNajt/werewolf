...
N_GPU = 4  # Use 4x A100 for 70B model
GPU_CONFIG = modal.gpu.A100(count=N_GPU)
...
app = modal.App("werewolf-apollo-probe-70b", image=image)
...
class ApolloProbeService:
    ...
    tokenizer: Any = None
    detector: Any = None
    detector_layer: int = 12  # Layer 12 for Llama 3.1 70B lie detection probe
    ...
    def load_model(self):
        ...
        self.llm = LLM(
            ...
            max_model_len=4096,  # 70B needs shorter context
        ...
        # Load default 70B detector if available
        default_detector_path = DETECTOR_DIR / "lie_detector_70b_layer12.pt"
        if default_detector_path.exists():
            self._load_detector(str(default_detector_path))
        else:
            print("No default 70B detector found. Use load_detector() to load one.")
...
def health_check():
    ...
    return {"status": "healthy", "service": "werewolf-apollo-probe-70b"}
